...
public class SimpleBulkPlacementStrategy implements IDPlacementStrategy {
    ...
    public void connectedComponentMaxProbGetPartitions(Map<InternalVertex, PartitionAssignment> vertices, List <Set <InternalVertex> > connectedComponents,List <Map <Long, Integer> > connectedComponents2partitionCount) {
    	List <Integer> connectedComponentMaxProbPartitionIds = new ArrayList<Integer>();
    	
    	for (Map <Long, Integer> partitionCount : connectedComponents2partitionCount) {
    		int partitionID = -1;
    		int partitionMaxCount = -1;
    		
    		for (Long partition : partitionCount.keySet()) {
    			if (partitionCount.get(partition) > partitionMaxCount) {
    				partitionMaxCount = partitionCount.get(partition);
    				partitionID = partition.intValue();
    			}
    		}
    		
    		if (partitionID == -1) {
    			connectedComponentMaxProbPartitionIds.add(nextPartitionID());
    		} else {
    			connectedComponentMaxProbPartitionIds.add(partitionID);
    		}
    	}
    	
    	int index = 0;
    	for (Set <InternalVertex> connectedComponent : connectedComponents) {
